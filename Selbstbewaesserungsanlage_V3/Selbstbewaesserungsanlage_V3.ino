...
void pumpen(int moisture){
	Serial.println("begin pumpe");
	Serial.flush();
	digitalWrite(relayPin, HIGH);
	int pumptime = moisture - moistureThreshold;
	delay(pumptime*20);
	Serial.println("stop pumpe");
	Serial.flush();
	digitalWrite(relayPin, LOW);
	ADCSRA = 0;
	sleepFor(sleepAfterPump);
	ADCSRA = adcsra_save;
}
